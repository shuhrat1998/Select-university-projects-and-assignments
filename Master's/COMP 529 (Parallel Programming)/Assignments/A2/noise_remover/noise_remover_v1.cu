 ...
 * Naive version using global memory.
 */
...
__global__ void compute1 (unsigned char *image, float *north_deriv, float *south_deriv, float *west_deriv, float *east_deriv, float *diff_coef, int width, int height, float std_dev)
{
    ...
    if ( (i>0 && i<(height-1)) && (j>0 && j<(width-1)) )
    {
        ...
        north_deriv[k] = image[(i - 1) * width + j] - image[k];	// --- 1 floating point arithmetic operations
        south_deriv[k] = image[(i + 1) * width + j] - image[k];	// --- 1 floating point arithmetic operations
        west_deriv[k] = image[i * width + (j - 1)] - image[k];	// --- 1 floating point arithmetic operations
        east_deriv[k] = image[i * width + (j + 1)] - image[k];	// --- 1 floating point arithmetic operations
...
__global__ void compute2 (unsigned char *image, float *north_deriv, float *south_deriv, float *west_deriv, float *east_deriv, float *diff_coef, int width, int height, float lambda)
{
    ...
    if ( (i>0 && i<(height-1)) && (j>0 && j<(width-1)) )
    {
        ...
        diff_coef_south = diff_coef[(i + 1) * width + j];
        diff_coef_west = diff_coef[k];
        diff_coef_east = diff_coef[i * width + (j + 1)];
