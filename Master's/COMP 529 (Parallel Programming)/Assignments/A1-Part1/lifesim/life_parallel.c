 ...
 *     Modified by Shukhrat Khuseynov [01/03/20]
 *************************************************************/
...
int main(int argc,char **argv)
{
    ...
    // a parallel region (although not required)
    #pragma omp parallel num_threads(numthreads) shared(nx, ny, currWorld, nextWorld)
    {
    ...
    // not parallelizing other parts of initialization as asked
    ...
    //printing data for myself
    /*printf("\n*** Beginning: ***\n");
    for (i=0; i<nx; i++)
    {
        for (j=0; j<ny; j++)
        {
            printf("%d", currWorld[i][j]);
        }
        printf("\n");
    }*/
        ...
        #pragma omp parallel
        {
            #pragma omp single
            {
                #pragma omp task
                {
                  ...
                  population = 0;
                  #pragma omp parallel num_threads(numthreads) shared(nx, ny, currWorld, nextWorld) reduction(+:population)
