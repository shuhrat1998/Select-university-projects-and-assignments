// SpMV Code - Part 2 (parallel with MPI & OpenMP)
// Created: 03-12-2019
// Author: Najeeb Ahmad
// Updated: 13-05-2020
...
int main(int argc, char **argv)
{
    ...
    if (myrank == 0)
    {
        ...
        retCode = mm_read_unsymmetric_sparse(argv[2], &matrix.m, &matrix.n, &matrix.nnz,
    ...
    if (myrank == 0)
        #pragma omp parallel for num_threads(numthreads) if(numthreads>1) shared(M, rowptr, matrix)
        for (int i=0; i<(M + 1); i++)
    ...
    #pragma omp parallel for num_threads(numthreads) if(numthreads>1) shared(N, rhs)
    for(int i = 0; i < N; i++)
      rhs[i] = (double) 1.0/N;
    ...
    {
        #pragma omp parallel for num_threads(numthreads) if(numthreads>1) shared(myrank, i0, rows, rowptr, my_colidx, my_val, rhs, result)
