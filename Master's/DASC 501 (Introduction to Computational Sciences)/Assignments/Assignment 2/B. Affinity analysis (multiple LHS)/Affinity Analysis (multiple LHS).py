...
Course: DASC 501
Assignment 2B / Affinity analysis (multiple LHS)
...
if Part == 1:
    ...
    """ Generating rules saved as a list of tuples, such that a -> y. """
    rules = [(a,y) for a in range(N) for y in range(N) if y!=a]

    """ Adding rules saved as a list of tuples, such that a, b -> y. """
    rules += [(a,b,y) for a in range(N) for b in range(a+1, N) for y in range(N) if y!=a and y!=b] 

    """ Adding rules saved as a list of tuples, such that a, b, c -> y. """
    rules += [(a,b,c,y) for a in range(N) for b in range(a+1, N) for c in range(b+1, N) for y in range(N) if y!=a and y!=b and y!=c]
    
    """ Adding rules saved as a list of tuples, such that a, b, c, d -> y. """
    rules += [(a,b,c,d,y) for a in range(N) for b in range(a+1, N) for c in range(b+1, N) for d in range(c+1, N) for y in range(N) if y!=a and y!=b and y!=c and y!=d] 
    ...
    f = open("Rules(B).txt", "w")
    f.write("DASC501\t   Affinity analysis (multiple LHS)\t@author: Shukhrat Khuseynov\n\n")
    f.write("The rules:\n")
    i = 1
    for r in rules:
        f.write("%d. " %i)
    ...
    f.close()
    print("\nThe output is saved in the Rules(B).txt file.")
else:
    print("Wrong input!")
    exit()
